...
create-environ:
	...
	VIRTUAL_ENV=${VIRTUAL_ENV} uv pip install openapi-python-client==${GENERATOR_VERSION}

generate:
	${VIRTUAL_ENV}/bin/openapi-python-client update --path schema.yaml --custom-template-path=templates --config config.yaml --meta poetry
